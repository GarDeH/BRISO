...
class get_ed_avg():
    ...
    def kon_2(self):
        Функция для определения объёма конической сверхзвуковой части сопла и теплозащитного попрытия.
                ...
                ----------------------------------------------------------
    Класс get_powder используется для определения параметров ДУ для звездообразного заряда или щелевого типа.
    Methods
    -------
    fi_kappa(kappa):
...
class get_p_graph():
    def __init__(self, INIT):
        ...
        if INIT['shape'] == 'slot':
            ...
            self.F_cut = INIT['F_cut']
            print(self.D_ks, self.F_cut, self.f_cut)
        if INIT['shape'] == 'star':
            self.flag = INIT['shape']
            self.r_ = INIT['r_']
            self.alpha_1 = INIT['alpha_1']
        ...
        print(self.D_ks, self.l_gr, self.omega, self.ro_t)
